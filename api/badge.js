...
async function getAgentLevel(address) {
  try {
    const r = await fetch(`${AIBTC_API}/agents/${address}`, { signal: abort(5000) });
    if (!r.ok) return null;
    const data = await r.json();
    if (!data.found || !data.levelName) return null;
    return data.level != null ? `${data.levelName} ${data.level}` : data.levelName;
  } catch { return null; }
}
...
function buildBadge({ tokenId, count, tier, agentName, alias, address, btcAddress, profileUrl, agentLevel }) {
  ...
  <g clip-path="url(#${uid}cl)">
    ...
    <a href="https://aibtc.com/agents/${esc(address)}" target="_blank">
      <text x="${W - 70}" y="37"
            font-family="-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif"
            font-size="7.5" fill="#8ba4c4" text-anchor="end" opacity="0.75">aibtc ↗</text>
    </a>

    <a href="${esc(profileUrl)}" target="_blank">
      <text x="${W - 8}" y="37"
            font-family="-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif"
            font-size="7.5" fill="${t.text}" text-anchor="end" opacity="0.85">eagle ↗</text>
    </a>
